...
func ComposeUploadFileHandler(w http.ResponseWriter, r *http.Request) {
	...
	if req.Name == "" || req.Content == "" {
		utils.WriteJSON(w, http.StatusBadRequest, types.ErrorResponse{Error: "name and content required"}); return
	}
	base, err := ComposeBaseDir()
	if err != nil { utils.WriteJSON(w, http.StatusInternalServerError, types.ErrorResponse{Error: err.Error()}); return }
	dest, err := SafeJoin(base, req.Name)
	if err != nil { utils.WriteJSON(w, http.StatusBadRequest, types.ErrorResponse{Error: err.Error()}); return }
...
func ComposeGetFileHandler(w http.ResponseWriter, r *http.Request) {
	...
	if path == "" {
		utils.WriteJSON(w, http.StatusBadRequest, types.ErrorResponse{Error: "path required"}); return
	}
	...
	if len(absTarget) < len(absBase) || absTarget[:len(absBase)] != absBase {
		utils.WriteJSON(w, http.StatusBadRequest, types.ErrorResponse{Error: "path escapes base"}); return
	}
...
func ComposeRun(w http.ResponseWriter, subcmd string, req types.ComposeRunRequest) {
	...
	if filePath == "" {
		utils.WriteJSON(w, http.StatusBadRequest, types.ErrorResponse{Error: "file_path required"}); return
	}
	workDir := req.WorkDir
	...
	if err != nil {
		utils.WriteJSON(w, http.StatusInternalServerError, map[string]any{"success": false, "output": string(out), "error": err.Error()}); return
	}
...
func ComposeUpHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err := json.NewDecoder(r.Body).Decode(&req); err != nil { utils.WriteJSON(w, http.StatusBadRequest, types.ErrorResponse{Error: "invalid JSON body"}); return }
...
func ComposeScaleHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err := json.NewDecoder(r.Body).Decode(&req); err != nil { utils.WriteJSON(w, http.StatusBadRequest, types.ErrorResponse{Error: "invalid JSON body"}); return }
	if req.Service == "" || req.Replicas < 0 { utils.WriteJSON(w, http.StatusBadRequest, types.ErrorResponse{Error: "service and replicas required"}); return }
